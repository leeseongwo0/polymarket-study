...
| 5 | 멘티 강의 가이드 | [`docs/mentee-learning-guideline.md`](docs/mentee-learning-guideline.md) | 멘토가 그대로 진행할 수 있는 원데이 강의 대본과 체크리스트를 제공합니다. |
...
docs/mentee-learning-guideline.md 멘토가 그대로 강의할 수 있는 멘티 학습 가이드
```
...
5. 멘토 진행이 필요하다면 [`docs/mentee-learning-guideline.md`](docs/mentee-learning-guideline.md)의 세션별 대본을 따라갑니다.
6. AI가 코드를 바꾸면 `git diff` 또는 에디터 diff를 읽고, `npm run test`부터 확인합니다.
7. 마지막에 `npm run lint && npm run test && npm run build && npm run test:e2e`를 실행합니다.
